...
- **Asymmetry proves it's a framework defect, not an app bug (my poc-ic1-asym.js)**: identical attacker input `?v=%0Abad` reflected into `x-echo` — via the **handler/sync path → graceful 500, server SURVIVES (exit 0)**; via an **async `onSend` hook → process CRASH (exit 1)**. Fastify recovers from an invalid header everywhere except the async hook-runner continuation. Not documented (docs only cover hooks *throwing to signal an error* → graceful 500).
